...
def create_daily_report(trade_date=-1, update_DB=False, market="CN", send_report=True, send_infographic=True):
    ...
    """
    df_bullishness_overview_master=df_bullishness_overview_master[a_real_cols]
    ...
    for asset in  d_a_assets[market]:
        ...
        for min_period, max_period in a_period[asset]:
            ...
            ideal_order_column = ["name", "period", "offensive_rank", "defensive_rank", "allround_rank_geo", "qdii_rank", "opportunity_rank", "investment_rank","total_mv"]
...
def loop():
    ...
    while True:
        ...
        day=now_time.day
        hour=now_time.hour
        print(f"time now is {now_time}")


        if hour==16:
            df_filter=df[(df["year"]==year)&(df["month"]==month)&(df["hour"]==hour)&(df["day"]==day)]
            if len(df_filter)==0:
    ...
    if do==2:
        for pf in ["p","f"]:
            ...
            for trade_date in df_trade_date.index:
                ...
                for freq in [60]:
                    for column in ["offensive_rank","defensive_rank","M_std","000001.SH_beta","399006.SZ_beta","399001.SZ_beta","pe_ttm","pb","pe_ttm_ALL","pb_ALL","total_mv","qdii_rank","close"]:
